...
class ShieldTest {
	...
	private Capacity shield = null;
	
	@BeforeEach
	public void initializeSword() {
		shield = new Sword(ANY_CHARC);
	}
	

	@Test
	void WHEN_ShieldIsCreated_THEN_ShieldHasCharacteristic() {
		//Assert
		assertEquals(ANY_CHARC, shield.getCharc());
	}
	
	@Test 
	void GIVEN_Shield_WHEN_AskedForPower_THEN_CalculatesPower() {
		FighterStat fighterStatMock = new FighterStatMock(ANY_SP, ANY_DP, ANY_IP, ANY_CP);
		//Assert
		final int EXPECTED_POWER = fighterStatMock.sp * shield.getCharc() / 100;
		assertEquals(EXPECTED_POWER, shield.calculatePower(shield, fighterStatMock));
	}
	...
	public void WHEN_CapacityIsCreated_and_InitialCharcValueIsTooLow_THEN_RaiseIllegalInitialCharcValueExceptionRightMessage() {
		...
		Assertions.assertThrows(IllegalInitialCharcValue.class, () -> {
			new Shield(Capacity.MIN_CHARC_VALUE - 1);
		    });	
	}
	...
	public void WHEN_CapacityIsCreated_and_InitialCharcValueIsTooHigh_THEN_RaiseIllegalInitialCharcValueExceptionRightMessage() {
		...
		//Act //Assert
		Assertions.assertThrows(IllegalInitialCharcValue.class, () -> {
			new Shield(Capacity.MAX_CHARC_VALUE + 1);
		    });	
	}
