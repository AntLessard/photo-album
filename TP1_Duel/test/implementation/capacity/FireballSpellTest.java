...
class FireballSpellTest {
	...
	private Capacity fireballSpell = null;
	
	@BeforeEach
	public void initializeFireballSpell() {
		fireballSpell = new FireballSpell(ANY_CHARC);
	}
	

	@Test
	void WHEN_FireballSpellIsCreated_THEN_FireballSpellHasCharacteristic() {
		//Assert
		assertEquals(ANY_CHARC, fireballSpell.getCharc());
	}
	
	@Test 
	void GIVEN_FireballSpell_WHEN_AskedForPower_THEN_CalculatesPower() {
		FighterStat fighterStatMock = new FighterStatMock(ANY_SP, ANY_DP, ANY_IP, ANY_CP);
		//Assert
		final int EXPECTED_POWER = (fighterStatMock.ip * fireballSpell.getCharc() / 100) * 3;
		assertEquals(EXPECTED_POWER, fireballSpell.calculatePower(fireballSpell, fighterStatMock));
	}
	...
	public void WHEN_CapacityIsCreated_and_InitialCharcValueIsTooLow_THEN_RaiseIllegalInitialCharcValueExceptionRightMessage() {
		...
		Assertions.assertThrows(IllegalInitialCharcValue.class, () -> {
			new FireballSpell(Capacity.MIN_CHARC_VALUE - 1);
		    });	
	}
	...
	public void WHEN_CapacityIsCreated_and_InitialCharcValueIsTooHigh_THEN_RaiseIllegalInitialCharcValueExceptionRightMessage() {
		...
		//Act //Assert
		Assertions.assertThrows(IllegalInitialCharcValue.class, () -> {
			new FireballSpell(Capacity.MAX_CHARC_VALUE + 1);
		    });	
	}
